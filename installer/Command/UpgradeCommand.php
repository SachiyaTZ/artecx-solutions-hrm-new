...
class UpgradeCommand extends InstallerCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->getIO()->caution(
            "IMPORTANT: Make sure it's a copy of the database of your current ARTecXHRM installation and not the original database. It's highly discouraged to use the original database for upgrading since it won't be recoverable if an error occurred during the upgrade."
        );
        $this->getIO()->warning(
        ...
        $this->getIO()->block(
            'Select your current ARTecXHRM version here. You can find the version at the bottom of the ARTecXHRM login page. ARTecXHRM Upgrader only supports versions listed in the dropdown. Selecting a different version would lead to an upgrade failure and a database corruption.'
        );
        ...
        if ($input->isInteractive() && $currentVersion == null) {
            $question = new ChoiceQuestion('Current ARTecXHRM Version ' . self::REQUIRED_TAG, $versions);
        ...
        $this->getIO()->title('Upgrading ARTecXHRM');
        $fromAndToVersions = "from <comment>ARTecXHRM $currentVersion</comment> to <comment>ARTecXHRM " . Config::PRODUCT_VERSION . '</comment>';
        ...
        if (!$input->isInteractive()) {
            $this->getIO()->info(
                "Upgrading from ARTecXHRM $currentVersion to ARTecXHRM " . Config::PRODUCT_VERSION . '.'
            );
        }
