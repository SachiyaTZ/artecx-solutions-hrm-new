...
class InstallOnNewDatabaseCommand extends InstallerCommand
{
    ...
    public const STEP_5 = 'Create ARTecXHRM database user';
    ...
    protected function licenseAcceptance(): void
    {
        ...
        $this->getIO()->block('Please review the license terms before installing ARTecXHRM Starter.');
    ...
    protected function databaseInformation(): void
    {
        ...
        $useSameDbUser = $this->getIO()->confirm(
            'Use the same `Privileged Database User` as `ARTecXHRM Database User`',
            false
        );
        ...
        if ($useSameDbUser === false) {
            $ohrmDbUser = $this->getRequiredField('ARTecXHRM Database Username');
            $ohrmDbPassword = $this->getIO()->askHidden(
                'ARTecXHRM Database User Password <comment>(hidden)</comment>'
            );
        }
        ...
        if (!$useSameDbUser && $this->getAppSetupUtility()->isDatabaseUserExist($ohrmDbUser)) {
            $this->getIO()->error(
                "Database User `$ohrmDbUser` Already Exist. Please Use Another Username for `ARTecXHRM Database Username`."
            );
            StateContainer::getInstance()->clearDbInfo();
            goto dbInfo;
        }
    ...
    protected function instanceCreation(): void
    {
        ...
        $this->getIO()->block(
            'Fill in your organization details here. Details entered in this section will be captured to create your ARTecXHRM Instance'
        );
    ...
    protected function adminUserCreation(): void
    {
        ...
        $regConsent = $this->getIO()->confirm(
            'Register your system with ARTecXHRM. By registering, You will be eligible for free support via emails, receive security alerts and news letters from ARTecXHRM.',
            true
        );
        StateContainer::getInstance()->storeRegConsent($regConsent);

        $this->getIO()->note(
            'Users who seek access to their data, or who seek to correct, amend, or delete the given information should direct their requests to info@artecx-solutions.com'
        );
    }
